...
const mergeText = (runs) =>
  runs.reduce((acc, run) => {
    ...
    return previous && previous.type === 'text' && run.type === 'text'
      ? [...acc.slice(0, -1), { type: 'text', value: previous.value + run.value }]
      : [...acc, run];
  }, []);
...
const isWordCharacter = (character) => character !== undefined && /\w/.test(character);
...
function parseInline(text) {
  ...
  while (rest.length > 0) {
    ...
    if (emphasis) {
      flush();
      runs.push({ type: emphasis.rule.type, runs: parseInline(emphasis.hit.inner) });
...
const linesToRuns = (lines) =>
  mergeText(
    lines.flatMap((line, index) => {
      ...
      return index === lines.length - 1 ? runs : [...runs, { type: 'text', value: ' ' }];
    })
  );
...
function readNote(lines, index) {
  ...
  return hit ? { block: { kind: 'note', text: hit[1].trim() }, next: index + 1 } : null;
}
...
function readHeading(lines, index) {
  ...
  return {
    block: { kind: 'heading', level: hit[1].length, runs: linesToRuns([hit[2]]) },
    next: index + 1,
  };
}
...
function readList(lines, index) {
  ...
  return { block: { kind: 'list', ordered, items: nest(clampLevels(items)) }, next: i };
}
